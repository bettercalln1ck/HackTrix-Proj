...
def upload_file(id):
	...
	total=UserProfile.objects.filter(user.is_superuser=True).exclude(filename='').count()
	users=UserProfile.objects.filter(user.is_superuser=True).exclude(filename='').order_by('space_used')[:parts]
